class MapController {
    constructor() {
        ...
        this.drawingTools = null; // Reference to DrawingTools instance
    }
        ...
        this.initPromise = new Promise((resolve, reject) => {
            try {
                this.map = new mapboxgl.Map({
                    ...
                    center: [73.0551, 33.6844], // San Francisco default
                    zoom: 1.5,
            ...
            this.geocoder = new MapboxGeocoder({
                ...
                types: 'place' // You can modify this to focus on 'address', 'poi', etc.
            });

            // Add geocoder to map
            this.map.addControl(this.geocoder, 'top-left');
            
            // Optional: You can listen to geocoder's events to interact with map
            this.geocoder.on('result', (e) => {
                console.log("Selected location:", e.result);
                // You can zoom to the selected result, for example
                this.map.flyTo({
    ...
    visualizePanels(panelGroupsData) {
        try {
            ...
            // Consider alignment line when visualizing panels
            if (this.drawingTools && this.drawingTools.alignmentLine) {
                const alignmentLine = this.drawingTools.alignmentLine;
                const lineAngle = this.calculateLineAngle(alignmentLine);
                
                // Adjust panel rotations based on alignment line
                newPanelFeatures.forEach(panel => {
                    if (!panel.properties) panel.properties = {};
                    panel.properties.rotation = lineAngle;
                });
            }
    ...
    calculateLineAngle(line) {
        if (!line || !line.geometry || !line.geometry.coordinates) return 0;
